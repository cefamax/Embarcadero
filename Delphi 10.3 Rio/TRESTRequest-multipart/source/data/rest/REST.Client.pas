...
type
  ...
  public
    ...
    /// </param>
    function AddItem(const AName, AValue: string; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions; AContentType: TRESTContentType): TRESTRequestParameter; overload;
    /// <summary>
    /// Adds a parameter to the request. If a parameter of the same name already exists, then the previous parameter
    /// will be removed and freed.<br /><br />There are five kinds of parameters: - GetOrPost: Either a QueryString
    /// value or encoded form value based on method - HttpHeader: Adds the name/value pair to the HTTP request's
    ...
    /// Content type of the parameter, for body parameters. Set to TRESTContentType.ctNone if no specific content type is needed.
    /// </param>
    function AddItem(const AName: string; const AValue: TBytes; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions; AContentType: TRESTContentType): TRESTRequestParameter; overload;
    /// <summary>
    /// Adds a parameter to the request. If a parameter of the same name already exists, then the previous parameter
    /// will be removed and freed.<br /><br />There are five kinds of parameters: - GetOrPost: Either a QueryString
    /// value or encoded form value based on method - HttpHeader: Adds the name/value pair to the HTTP request's
    ...
    /// </param>
    function AddItem(const AName: string; const AValue: TStream; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions; AContentType: TRESTContentType; AOwnsStream: TRESTObjectOwnership = ooCopy): TRESTRequestParameter; overload;
    /// <summary>
  ...
  public
    constructor Create(AExecuteMethod: TExecuteMethod; ARequest: TCustomRESTRequest; ACompletionHandler: TCompletionHandler; ASynchronized: Boolean; AFreeThread: Boolean = True; ACompletionHandlerWithError: TCompletionHandlerWithError = nil);
  end;
  ...
  private
    ...
    function IsQueryParam(const AParam: TRESTRequestParameter; AContentType: TRESTContentType): Boolean;
...
const
  sRequestDefaultAccept = CONTENTTYPE_APPLICATION_JSON + ', ' + CONTENTTYPE_TEXT_PLAIN + '; q=0.9, ' + CONTENTTYPE_TEXT_HTML + ';q=0.8,';
...
procedure TRESTRequestParameter.SetStream(const AValue: TStream; AOwnership: TRESTObjectOwnership);
begin
...
begin
  ...
  if Stream <> nil then
    ...
    LValue := FValue;
  Result := Format('[%s%s] %s=%s', [RESTRequestParameterKindToString(FKind), LEncode, FName, LValue])
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (LParam.Kind = TRESTRequestParameterKind.pkURLSEGMENT) and (TRESTRequestParameterOption.poAutoCreated in LParam.Options) then
      begin
...
begin
  case AIndex of
    ...
    2:
      Result := System.TypInfo.GetEnumName(TypeInfo(TRESTRequestParameterKind), Integer(Items[AItemIndex].Kind));
  else
    Result := '';
  end;
end;
...
function TRESTRequestParameterList.AddItem(const AName, AValue: string; AKind: TRESTRequestParameterKind; AOptions: TRESTRequestParameterOptions): TRESTRequestParameter;
begin
  Result := AddItem(AName, AValue, AKind, AOptions, DefaultRESTContentType);
end;

function TRESTRequestParameterList.AddItem(const AName, AValue: string; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions; AContentType: TRESTContentType): TRESTRequestParameter;
var
  i: Integer;
begin
  Self.BeginUpdate;
...
function TRESTRequestParameterList.AddItem(const AName: string; const AValue: TBytes; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions; AContentType: TRESTContentType): TRESTRequestParameter;
var
  i: Integer;
begin
  BeginUpdate;
...
function TRESTRequestParameterList.AddItem(const AName: string; const AValue: TStream; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions; AContentType: TRESTContentType; AOwnsStream: TRESTObjectOwnership): TRESTRequestParameter;
var
  i: Integer;
begin
  BeginUpdate;
...
constructor TRESTExecutionThread.Create(AExecuteMethod: TExecuteMethod; ARequest: TCustomRESTRequest; ACompletionHandler: TCompletionHandler; ASynchronized: Boolean; AFreeThread: Boolean = True; ACompletionHandlerWithError: TCompletionHandlerWithError = nil);
begin
...
procedure TRESTRequestParameterList.AddBody(const ABodyContent: string; AContentType: TRESTContentType);
var
  LGUID: TGUID;
  LGuidString: string;
  LBodyContent: string;
begin
  ...
  AddItem(LGuidString, LBodyContent, TRESTRequestParameterKind.pkREQUESTBODY, [], AContentType);
end;

procedure TRESTRequestParameterList.AddBody(const AObject: TJsonObject; AOwnsObject: TRESTObjectOwnership);
var
  LBodyContent: string;
begin
  try
    LBodyContent := TJson.JsonEncode(AObject);
    AddItem(sBody, LBodyContent, TRESTRequestParameterKind.pkREQUESTBODY, [], TRESTContentType.ctAPPLICATION_JSON);
  finally
...
procedure TRESTRequestParameterList.AddBody(const ABodyContent: TStream; AContentType: TRESTContentType; AOwnsStream: TRESTObjectOwnership);
begin
  AddItem(sBody, ABodyContent, TRESTRequestParameterKind.pkREQUESTBODY, [poDoNotEncode], AContentType, AOwnsStream);
end;

procedure TRESTRequestParameterList.AddBody<T>(const AObject: T; AOwnsObject: TRESTObjectOwnership = ooApp);
var
...
procedure TCustomRESTRequest.DoPrepareRequestBody(AParamList: TRESTRequestParameterArray; AContentType: TRESTContentType; var ABodyStream: TStream; var ABodyStreamOwner: Boolean);
var
  LParam: TRESTRequestParameter;
  LParamString: string;
  LParamName: string;
...
begin
  ...
  try
    ...
    begin
      if (LParam.Kind = TRESTRequestParameterKind.pkGETorPOST) and not IsQueryParam(LParam, AContentType) or (LParam.Kind in [TRESTRequestParameterKind.pkREQUESTBODY, TRESTRequestParameterKind.pkFile]) then
      begin
        ...
        begin
          ...
          if LParam.Stream <> nil then
            LMultipartFormData.AddStream(LParam.Name, LParam.Stream, LParam.Value, ContentTypeToString(LParam.ContentType))
          else if LParam.Kind = TRESTRequestParameterKind.pkFile then
            LMultipartFormData.AddFile(LParam.Name, LParam.Value, ContentTypeToString(LParam.ContentType))
          else
        ...
        else if LParam.Stream <> nil then
        begin
...
function TCustomRESTRequest.ExecuteAsync(ACompletionHandler: TCompletionHandler = nil; ASynchronized: Boolean = True; AFreeThread: Boolean = True; ACompletionHandlerWithError: TCompletionHandlerWithError = nil): TRESTExecutionThread;
begin
  ...
  try
    ...
    if FBody.FParams.Count > 0 then
      for LParam in FBody.FParams do
        ...
        if LParam.Stream <> nil then
          FParams.AddItem(LParam.FName, LParam.Stream, LParam.FKind, LParam.Options, LParam.ContentType, ooApp)
        else
...
//        xmemStr := TMemoryStream.Create;
//        xmemStr.CopyFrom(LBodyStream, 0);
//        xmemStr.SaveToFile('c:\pippo.txt');
//        xmemStr.Free;
//      end;
//      {$ENDIF}
      ...
      try
        ...
        try
          ...
          begin
            ...
            if (FClient.FallbackCharsetEncoding <> '') and not SameText(REST_NO_FALLBACK_CHARSET, FClient.FallbackCharsetEncoding) then
            begin
...
procedure TCustomRESTRequest.AddAuthParameter(const AName, AValue: string; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions);
begin
  AddParameter(AName, AValue, AKind, AOptions + [TRESTRequestParameterOption.poTransient]);
end;

procedure TCustomRESTRequest.AddBody(ABodyContent: TStream; AContentType: TRESTContentType = TRESTContentType.ctNone; AOwnsStream: TRESTObjectOwnership = TRESTObjectOwnership.ooCopy);
begin
...
procedure TCustomRESTRequest.AddParameter(const AName, AValue: string; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions);
begin
...
procedure TCustomRESTRequest.AddParameter(const AName: string; AJsonObject: TJsonObject; AOwnsObject: TRESTObjectOwnership { = ooREST } );
begin
...
function TCustomRESTRequest.IsQueryParam(const AParam: TRESTRequestParameter; AContentType: TRESTContentType): Boolean;
begin
  Result := (AParam.Kind = TRESTRequestParameterKind.pkGETorPOST) and ((Method in [TRESTRequestMethod.rmGET, TRESTRequestMethod.rmDELETE]) or (Method in [TRESTRequestMethod.rmPOST, TRESTRequestMethod.rmPUT, TRESTRequestMethod.rmPATCH]) and not(AContentType in [TRESTContentType.ctAPPLICATION_X_WWW_FORM_URLENCODED, TRESTContentType.ctMULTIPART_FORM_DATA])) or (AParam.Kind = TRESTRequestParameterKind.pkQUERY);
end;
...
begin
  ...
  begin
    if (Method in [TRESTRequestMethod.rmGET, TRESTRequestMethod.rmDELETE]) and not(LParam.Kind in [TRESTRequestParameterKind.pkREQUESTBODY, TRESTRequestParameterKind.pkFile]) then
      Continue;
  ...
  if LSpecificContentType <> ctNone then
    ...
    Result := TRESTContentType.ctMULTIPART_FORM_DATA
  else if not(Method in [TRESTRequestMethod.rmGET, TRESTRequestMethod.rmDELETE]) or (LNumBodyParams > 0) then
...
procedure TCustomRESTRequest.DoPrepareQueryString(const AParamList: TRESTRequestParameterArray; AContentType: TRESTContentType; var AURL: string);
var
  LParam: TRESTRequestParameter;
  LName: string;
  LValue: string;
...
begin
  ...
  if LFullRes > '' then
  begin
    // because there may be a trailing slash at the
    // end of the base-url, and also no trailing slash at the
    // beginning of a resource, so we may need to add one.
...
procedure TRESTRequestAdapter.AddParameterFields;
  ...
  procedure MakeParameterField(const AParam: TRESTRequestParameter; const AFieldName: string; const AGetMemberObject: IGetMemberObject);
  begin
...
procedure TCustomRESTResponse.GetJSONResponse(out AJSONValue: TJSONValue; out AHasOwner: Boolean);
var
  LJSONValue: TJSONValue;
...
procedure TCustomRESTResponse.RemoveJSONChangedEvent(const ANotify: TNotifyEvent);
begin
...
procedure TCustomRESTClient.AddAuthParameter(const AName, AValue: string; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions);
begin
  AddParameter(AName, AValue, AKind, AOptions + [TRESTRequestParameterOption.poTransient]);
end;

procedure TCustomRESTClient.AddParameter(const AName, AValue: string; const AKind: TRESTRequestParameterKind; const AOptions: TRESTRequestParameterOptions);
begin
  // decide where to put the parameter to
  if TRESTRequestParameterOption.poTransient in AOptions then
  begin
...
procedure TCustomRESTClient.SetOnValidateCertificate(const Value: TValidateCertificateEvent);
begin
...
constructor TCustomRESTResponse.EJSONValueError.Create(AError: TJSONValueError; const AMessage: string);
begin
...
procedure TCustomRESTRequest.TBody.Add(const ABodyContent: string; AContentType: TRESTContentType);
begin
  FParams.AddBody(ABodyContent, AContentType);
end;

procedure TCustomRESTRequest.TBody.Add(ABodyContent: TStream; AContentType: TRESTContentType; AOwnsStream: TRESTObjectOwnership);
begin
  FParams.AddBody(ABodyContent, AContentType, AOwnsStream);
end;
