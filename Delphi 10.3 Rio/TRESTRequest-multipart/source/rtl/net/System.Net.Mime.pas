...
type
  ...
  public type
    TAcceptFunc = reference to function(const AName: string; AWeight: Double; AItem: T): Boolean;
  private type
  ...
  private
    ...
    function InternalNegotiate(AAcceptList: TAcceptValueListBase<T>; AAcceptFunc: TAcceptFunc; var AMode: TMatchMode; out AWeight: Double): string;
  public
    ...
    function Negotiate(const AAcceptList: TAcceptValueListBase<T>; out AWeight: Double; AAcceptFunc: TAcceptFunc): string; overload;
    function Negotiate(const AAcceptList: string; out AWeight: Double; AAcceptFunc: TAcceptFunc): string; overload;
...
procedure TMultipartFormData.AddStream(const AField: string; AStream: TStream; const AFileName: string; const AContentType: string);
var
  LLine, LType: string;
...
procedure TMultipartFormData.AddBytes(const AField: string; const ABytes: TBytes; const AFileName, AContentType: string);
var
...
begin
  ...
  begin
    // max FStream.Position := FStream.Size - (Length(FBoundary) + 4);
...
{$REGION 'MIME CONST'}
  ...
  { } AddType('js', 'text/plain', TMimeTypes.TKind.Text); // do not localize
...
function TMimeTypes.GetFileInfo(const AFileName: string; out AType: string; out AKind: TKind): Boolean;
begin
  Result := GetExtInfo(ExtractFileExt(AFileName), AType, AKind);
end;

function TMimeTypes.GetExtInfo(const AExt: string; out AType: string; out AKind: TKind): Boolean;
var
  LExt: string;
  LInfo: TInfo;
begin
...
function TMimeTypes.GetTypeInfo(const AType: string; out AExt: string; out AKind: TKind): Boolean;
var
  LInfo: TInfo;
  LType: string;
begin
...
begin
  ...
  try
    ...
    for LInfo in FInfos.Values do
      if ((LExtMask = nil) or LExtMask.Matches(LInfo.FExt)) and ((LTypeMask = nil) or LTypeMask.Matches(LInfo.FType)) then
...
class function TAcceptValueListBase<T>.CompareWeights(AWeight1, AWeight2: Double): Integer;
begin
...
procedure TAcceptValueListBase<T>.Add(const AName: string; AWeight: Double; AExtra: TStrings);
var
  LItem: T;
  LName, LType: string;
  LIndex: Integer;
...
function TAcceptValueListBase<T>.InternalNegotiate(AAcceptList: TAcceptValueListBase<T>; AAcceptFunc: TAcceptFunc; var AMode: TMatchMode; out AWeight: Double): string;
var
  i, j: Integer;
  LMask1: TMask;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (LNewWeight <> 0) and ((LMask1 <> nil) and LMask1.Matches(LName2) or (LMask1 = nil) and (LName1 = LName2)) and (not Assigned(AAcceptFunc) or AAcceptFunc(LName2, LNewWeight, AAcceptList.Items[j])) then
...
function TAcceptValueListBase<T>.Negotiate(const AAcceptList: TAcceptValueListBase<T>; out AWeight: Double; AAcceptFunc: TAcceptFunc): string;
var
  LItem: string;
  LWeight: Double;
  LMode: TMatchMode;
...
function TAcceptValueListBase<T>.Negotiate(const AAcceptList: string; out AWeight: Double; AAcceptFunc: TAcceptFunc): string;
var
...
begin
  ...
  try
    LList.BeginUpdate;
    try
      LFunc := function(const AName: string; AWeight: Double; AItem: T): Boolean
        begin
